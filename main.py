...
# 🤖 ML PREDICTION (ONLY CHANGE)
def predict_risk(rain_percent):

    pred = model.predict([[rain_percent]])[0]

    # 🔥 dynamic score (realistic bar)
    if pred == 0:
        return "Normal", round(rain_percent * 0.5, 2)
    elif pred == 1:
        return "Risk", round(rain_percent * 0.8, 2)
    else:
        return "High Risk", min(100, round(rain_percent * 1.2, 2))
